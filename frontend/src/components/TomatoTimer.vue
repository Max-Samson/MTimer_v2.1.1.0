...
import { NButton, NIcon, NRadioButton, NRadioGroup, NSpace, NTag, NTooltip } from 'naive-ui'
...
function playFallbackSound(frequency: number = 440, duration: number = 500): void {
  try {
    // 创建音频上下文
...
<template>
  <div class="tomato-timer">
    <!-- 番茄藤蔓装饰 -->
    <div class="tomato-vine left-vine" />
    <div class="tomato-vine right-vine" />
    ...
    <div class="tomato-status" :class="{ 'status-active': isRunning }">
      <div class="status-left">
        ...
        <NTooltip trigger="hover" placement="bottom">
          <template #trigger>
            <NButton text size="small" class="info-btn" @click="showPomodoroInfo">
              <NIcon>
                <Help />
              </NIcon>
            </NButton>
      ...
      <NTag :type="isRunning ? 'success' : 'warning'" size="small" class="status-tag">
        {{ statusText }}
      </NTag>
    </div>
    ...
    <div v-if="currentTodo" class="current-todo-info">
      <div class="todo-title">
        ...
        <NTooltip placement="top">
          <template #trigger>
            <NTag type="info">
              {{ currentTodo.text || currentTodo.name }}
            </NTag>
          </template>
    ...
    <div class="timer-display" :class="{ 'timer-active': isRunning }">
      ...
      <TomatoSvg :time="time" :is-running="isRunning" />
    </div>
    ...
    <div class="progress-bar-container">
      ...
      <div class="progress-bar">
        ...
        <div class="progress-tomato-icon" :style="{ left: `${progress}%` }" />
      </div>
      <div class="progress-label">
        {{ isBreak ? '放松一下!' : 'Time to focus!' }}
      </div>
    </div>
    ...
    <div class="controls">
      <NSpace justify="center">
        <NButton
          ...
          class="control-btn toggle-btn" @click="isRunning ? stopTimerWithEffects() : startTimer()"
        >
          <span>{{ isRunning ? '暂停专注' : '开始专注' }}</span>
        </NButton>
        <NButton type="info" class="control-btn toggle-btn reset-btn" @click="resetTimer">
          <span>重置时间</span>
        </NButton>
      </NSpace>
    </div>
    ...
    <div class="mode-switch">
      <NRadioGroup v-model:value="currentMode" size="small" @update:value="handleModeChange">
        <NRadioButton value="pomodoro">
          番茄工作法
        </NRadioButton>
      ...
      <NButton
        v-if="currentMode === 'custom'" text type="primary" class="settings-btn"
    ...
    <div class="timer-info">
      <NSpace justify="center">
        <NTag v-if="!isBreak" type="success">
